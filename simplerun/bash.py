...
from simplerun.events.action import CmdRunAction
from simplerun.events.observation import (
    CmdOutputObservation,
    ErrorObservation,
    CmdOutputMetadata,
)
from simplerun.events.observation.commands import CMD_OUTPUT_PS1_END
...
class BashSession: # 完整的bash会话管理系统，通过tmux提供持久的bash环境，支持命令执行、状态跟踪和超时管理。
    ...
    def initialize(self) -> None: # 创建和配置tmux会话，设置bash环境
        ...
        if self.username in ['root', 'simplerun']:
        ...
        session_name = f'simplerun-{self.username}-{uuid.uuid4()}'
